...
exports.up = async (queryInterface, _) => {
    ...
    await queryInterface.sequelize.query(`INSERT INTO tags (name, icon)
    ...
    await queryInterface.sequelize.query(`INSERT INTO articles (title, content, tags)
...
exports.down = async (queryInterface, _) => {
    await queryInterface.sequelize.query(`DELETE FROM users WHERE email = rauchg@vercel.com`);
    await queryInterface.sequelize.query(`DELETE FROM users WHERE email = lee@vercel.com`);
    await queryInterface.sequelize.query(`DELETE FROM users WHERE email = stey@vercel.com`);
    await queryInterface.sequelize.query(`DELETE FROM tags WHERE name = women`);
    await queryInterface.sequelize.query(`DELETE FROM tags WHERE name = poll`);
    await queryInterface.sequelize.query(`DELETE FROM articles WHERE title = $$Without women, you won''t win these elections$$`);
    await queryInterface.sequelize.query('DELETE FROM articles WHERE title = $$Latest Election Polls - Who Will Be the Third Force in the Parliament?$$');
    await queryInterface.sequelize.query('DELETE FROM questions WHERE text = Oppression by corporations is more of a concern than oppression by governments.');
    await queryInterface.sequelize.query('DELETE FROM questions WHERE text = It is necessary for the government to intervene in the economy to protect consumers.');
    await queryInterface.sequelize.query('DELETE FROM questions WHERE text = The freer the markets, the freer the people.');
    await queryInterface.sequelize.query('DELETE FROM questions WHERE text = It is better to maintain a balanced budget than to ensure welfare for all citizens.');
    await queryInterface.sequelize.query('DELETE FROM questions WHERE text = Publicly-funded research is more beneficial to the people than leaving it to the market.');
    await queryInterface.sequelize.query('DELETE FROM questions WHERE text = Tariffs on international trade are important to encourage local production.');
    await queryInterface.sequelize.query('DELETE FROM questions WHERE text = From each according to his ability, to each according to his needs.');
    await queryInterface.sequelize.query('DELETE FROM questions WHERE text = It would be best if social programs were abolished in favor of private charity.');
    await queryInterface.sequelize.query('DELETE FROM questions WHERE text = Taxes should be increased on the rich to provide for the poor.');
    await queryInterface.sequelize.query('DELETE FROM parties WHERE name = Pizza Friends');
    await queryInterface.sequelize.query('DELETE FROM parties WHERE name = Promise And Conquer');
    await queryInterface.sequelize.query('DELETE FROM parties WHERE name = Do Whatever You Want');
    await queryInterface.sequelize.query('DELETE FROM parties WHERE name = We Are The Champions');
};
